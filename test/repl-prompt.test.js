...
// node:test mock.timers gained the { apis } options object in Node 20.4; on
// Node 18 (still supported per package.json engines) it only accepts an array,
// so the timer-driven spinner test below is skipped there — it exercises a
// render detail, not runtime behaviour.
const nodeMajor = Number(process.versions.node.split('.')[0]);
const needsTimerApis = nodeMajor < 20 ? 'mock.timers { apis } needs Node 20.4+' : false;
...
test(
  'thinking() repaints the spinner without redrawing the prompt (no cursor flash)',
  { skip: needsTimerApis },
  () => {
